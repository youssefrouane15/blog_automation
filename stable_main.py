...
class KeywordProcessor:
    ...
    def get_internal_links(self, website_config: Dict, keyword: str, num_links: int = 3) -> List[Dict]:
        try:
            ...
            if response.status_code != 200:
                self.logger.error(f"Failed to fetch sitemap from {sitemap_url}")
                return []
            ...
            if root.tag.endswith('sitemapindex'):
                sitemap_urls = [child.find('sm:loc', namespace).text for child in root.findall('sm:sitemap', namespace)]
                ...
                for sitemap_url in sitemap_urls:
                    try:
                        ...
                        self.logger.error(f"Error processing sub-sitemap {sitemap_url}: {str(e)}")
            else:
            ...
            filtered_urls = []
            for url in urls:
                if any(segment in url for segment in ['/category/', '/tag/', '/author/', '/page/']):
                    continue
                    
                if url.endswith(('.xml', '.php', '.jpg', '.png', '.pdf')):
                    continue
                    
                parts = urlparse(url)
                path_segments = parts.path.strip('/').split('/')
                if not path_segments or parts.path == '/':
                    continue
            ...
            for url in filtered_urls:
                if any(term in url.lower() for term in ["guide", "ultimate", "complete", "basics", "101"]):
                    ...
                    if relevance_score > 0 or url in pillar_content:
                        try:
                            ...
                            if page_resp.status_code == 200:
                                ...
                                title_text = title.get_text() if title else url.split('/')[-1].replace('-', ' ').title()
            ...
            return related_links[:num_links]
            
        except Exception as e:
            self.logger.error(f"Error finding internal links: {str(e)}")
            return []
...
class ContentGenerator:
    ...
    def generate_recipe_card(self, content: str, focused_keyword: str, recipe_data: Dict) -> Dict:
        """
        Extract recipe details from the generated article content and generate SEO metadata for RankMath
        
        Args:
            content: HTML content
            focused_keyword: Focus keyword
            recipe_data: Recipe data from SERP analysis (used as fallback)
            
        Returns:
            Dict with recipe card data and SEO metadata
        """
        try:
            # Use Claude to extract and format the entire recipe card with SEO metadata
            prompt = f"""
            You are a recipe data extraction and SEO expert. I need you to analyze this recipe article and extract the complete recipe card in structured format, along with SEO metadata for RankMath.
            MANDATORY NOTE: RESPECT THE SPECIFICATION FOR EACH FIELD, stick to what I specify, the rules are important !
            Here's the article content:
            {content[:12000]}

            Focus keyword: {focused_keyword}

            Please extract the following data in exact JSON format with these specific fields:

            {{
            "Recipe_Title": "",  # Extract the main recipe title and optimize it to 5-8 SEO-friendly words
            "Recipe_Description": "",  # Create a 1-2 sentence SEO-optimized description capturing the essence of the recipe
            "Recipe_Keywords": "",  # Give 5-8 relevant keywords,separate the keywords by a comma, ensuring "{focused_keyword}" is included in one keyword at max, the keywords serve as alternatives to this recipe, or complementary infos about the recipe
            "Recipe_Details": {{
                "Prep_Time": "",  
                "Cook_Time": "",  
                "Total_Time": "", 
            ...
            "ingredients_html": "",  # CRITICAL: Format ALL ingredients as proper HTML with <ul> and <li> tags. Each ingredient MUST be wrapped in <li> tags
            "instructions_html": "", # CRITICAL: Format ALL instruction steps as proper HTML with <ol> and <li> tags. Each step MUST be wrapped in <li> tags  
            "Recipe_Ingredients": [],  # Also provide ingredients as a simple array for backup
            "Recipe_Instructions": [], # Also provide instructions as a simple array for backup
            "Recipe_Notes": "",  # Format as HTML with <ul> and <li> tags for multiple notes
            "Recipe_Nutrition": {{
                "Serving_Size": "",
                "Calories": "",
                "Sugar": "",
            ...
            "SEO_Metadata": {{
                "seo_title": "",  # Create an optimal SEO title of EXACTLY 55-60 characters including the focus keyword at the beginning, use a power word like "Best", "Easy", "Delicious", "Perfect", "Amazing", avoid AI words
                "seo_description": "",  # Create a compelling meta description of EXACTLY 150-160 characters including the focus keyword naturally
                "slug": "",  # Create a URL-friendly slug of 3-5 words MAX, using hyphens to separate words, based on the focus keyword only
                "focus_keyword": "{focused_keyword}",  # Use this exact focus keyword
                "focus_keyword_variants": []  # Create 3-5 focus keyword variants as an array of strings, should be closely related to the main focus keyword
            }}
            }}

            CRITICAL FORMATTING INSTRUCTIONS:
            1. For ingredients_html: 
            - MUST start with <ul class="recipe-ingredients">
            - Each ingredient MUST be wrapped in <li> tags
            - Extract ALL ingredients from the article
            - Example: <ul class="recipe-ingredients"><li>2 cups flour</li><li>1 tsp salt</li></ul>

            2. For instructions_html:
            - MUST start with <ol class="recipe-instructions">
            - Each step MUST be wrapped in <li> tags
            - Include all steps from the article
            - Example: <ol class="recipe-instructions"><li>Preheat oven to 350°F</li><li>Mix dry ingredients</li></ol>

            3. SEO Rules (MANDATORY):
            - seo_title: MUST be 55-60 characters, start with focus keyword, include power word
            - seo_description: MUST be 150-160 characters, include focus keyword naturally
            - slug: MUST be 3-5 words ONLY, hyphenated, based on focus keyword

            Return ONLY valid JSON without explanations, comments or markdown formatting.
            """
            
            # Call Claude API
            response = self.anthropic_client.messages.create(
                model=self.config.config['claude_recipe_card_model'],
                temperature=0.2,
                max_tokens=1500,
                messages=[{"role": "user", "content": prompt}]
            )
            ...
            # Validate and ensure HTML formatting for ingredients and instructions
            if 'ingredients_html' in recipe_card and not recipe_card['ingredients_html'].startswith('<ul'):
                # Convert to proper HTML if needed
                ingredients = recipe_card.get('Recipe_Ingredients', [])
                recipe_card['ingredients_html'] = '<ul class="recipe-ingredients">' + ''.join([f'<li>{ing}</li>' for ing in ingredients]) + '</ul>'
            
            if 'instructions_html' in recipe_card and not recipe_card['instructions_html'].startswith('<ol'):
                # Convert to proper HTML if needed
                instructions = recipe_card.get('Recipe_Instructions', [])
                recipe_card['instructions_html'] = '<ol class="recipe-instructions">' + ''.join([f'<li>{inst}</li>' for inst in instructions]) + '</ol>'
            
            # Log successful extraction
            self.logger.info(f"Recipe card and SEO metadata successfully generated for: {recipe_card.get('Recipe_Title', focused_keyword)}")
            
            return recipe_card
                
        except Exception as e:
            self.logger.error(f"Error generating recipe card and SEO metadata: {str(e)}")
            
            # Fallback to a basic recipe card with SEO metadata
            return self._create_fallback_recipe_card_with_seo(focused_keyword, recipe_data)

    def _create_fallback_recipe_card_with_seo(self, focused_keyword: str, recipe_data: Dict) -> Dict:
       ingredients = recipe_data.get("ingredients", ["Ingredient 1", "Ingredient 2", "Ingredient 3"])
       instructions = recipe_data.get("instructions", ["Step 1", "Step 2", "Step 3"])
       
       return {
           "Recipe_Title": f"{focused_keyword.title()} Recipe",
           "Recipe_Description": f"A delicious recipe for {focused_keyword}.",
           "Recipe_Notes": "<ul><li>Store in an airtight container for up to 3 days.</li></ul>",
           "Recipe_Keywords": f"{focused_keyword}, recipe, homemade, cooking, food",
           "Recipe_Details": {
               "Prep_Time": recipe_data.get("prep_time", "20 minutes"),
               "Cook_Time": recipe_data.get("cook_time", "30 minutes"),
               "Total_Time": recipe_data.get("total_time", "50 minutes"),
               "Yield": recipe_data.get("servings", "4 servings"),
               "Category": "Main Course",
               "Method": "Cooking",
               "Cuisine": "International"
           },
           "ingredients_html": '<ul class="recipe-ingredients">' + ''.join([f'<li>{ing}</li>' for ing in ingredients]) + '</ul>',
           "instructions_html": '<ol class="recipe-instructions">' + ''.join([f'<li>{inst}</li>' for inst in instructions]) + '</ol>',
           ...
           "SEO_Metadata": {
               "seo_title": f"Best {focused_keyword.title()} Recipe - Easy & Delicious",
               "seo_description": f"Learn how to make the perfect {focused_keyword} with our easy step-by-step recipe. This delicious homemade {focused_keyword} is ready in just 45 minutes!",
               "slug": focused_keyword.lower().replace(" ", "-")[:50],
               "focus_keyword": focused_keyword,
               "focus_keyword_variants": [
                   f"homemade {focused_keyword}",
                   f"best {focused_keyword} recipe",
                   f"easy {focused_keyword}",
                   f"how to make {focused_keyword}"
               ]
           }
       }
    ...
    async def generate_seo_metadata(self, keyword: str, serp_data: Dict) -> Dict:
       try:
           seo_params = self.config.get_seo_params()
           max_focus_keyword_words = seo_params.get('max_focus_keyword_words', 4)
           
           recipe_title = serp_data.get('title', f"{keyword.title()} Recipe")
           recipe_snippet = serp_data.get('snippet', f"A delicious recipe for {keyword}.")
           ...
           CRITIAL SEO RULES TO FOLLOW, It's mandatory and very important to stick to the rules:
           - Focus keyword: 2-{max_focus_keyword_words} words maximum, highly specific
           - SEO title: exactly between 60-65 characters maximum, include focus keyword at the beginning, a powerful positive word at the beginning is mandatory.
           - Meta description: exactly 150 characters exactly, include focus keyword and avoid AI words such as indluge etc, try to use something human-like
           - URL slug: exactly between 50-60 characters, kebab-case (lowercase with hyphens)
           ...
           focus_keyword = metadata.get("focus_keyword", "").strip()
           if not focus_keyword:
               focus_keyword = keyword
           
           word_count = len(focus_keyword.split())
           if word_count > max_focus_keyword_words:
               self.logger.warning(f"Focus keyword '{focus_keyword}' exceeds max words. Truncating.")
               focus_keyword = ' '.join(focus_keyword.split()[:max_focus_keyword_words])
           ...
           max_slug_length = seo_params.get('max_slug_length', 80)
           if len(slug) > max_slug_length:
               slug = slug[:max_slug_length].rstrip('-')
           
           meta_description = metadata.get("meta_description", "").strip()
           max_desc_length = seo_params.get('max_meta_description_length', 160)
           if len(meta_description) > max_desc_length:
               meta_description = meta_description[:max_desc_length].rstrip()
           
           validated_metadata = {
               "focus_keyword": focus_keyword,
               "seo_title": metadata.get("seo_title", f"{focus_keyword.title()} Recipe"),
           ...
           return {
               "focus_keyword": keyword,
               "seo_title": f"{keyword.title()} Recipe",
               "meta_description": f"Discover how to make the perfect {keyword} with this easy recipe. Step-by-step instructions and tips for a delicious homemade meal.",
    ...
    async def generate_content(self, keyword: str, focused_keyword: str, serp_data: Dict, 
      ...
      try:
          ...
          recipe_data = serp_data.get("recipe_data", {})
          style_analysis = serp_data.get("style_analysis", {})
...
class WordPressPublisher:
    ...
    def generate_metadata(self, keyword: str, content: str, focused_keyword: str, serp_data: Dict, seo_metadata: Dict) -> ArticleMetadata:
      try:
          recipe_card_data = None
          
          if self.content_generator:
              recipe_card_data = self.content_generator.generate_recipe_card(
                  content, focused_keyword, serp_data.get("recipe_data", {})
              )
              
          if not recipe_card_data:
              recipe_card_data = self._create_fallback_recipe_card(keyword, serp_data.get("recipe_data", {}))
          
          soup = BeautifulSoup(content, 'html.parser')
          h1_tag = soup.find('h1')
          title = h1_tag.get_text().strip() if h1_tag else f"{focused_keyword.title()} Recipe"
          
          recipe_seo = recipe_card_data.get("SEO_Metadata", {}) if isinstance(recipe_card_data, dict) else {}


          title = recipe_seo.get("seo_title", seo_metadata.get("seo_title", f"{focused_keyword.title()} Recipe"))
          meta_description = recipe_seo.get("seo_description", seo_metadata.get("meta_description", ""))
          slug = recipe_seo.get("slug", seo_metadata.get("slug", ""))

          if not slug:
            slug = focused_keyword.lower().replace(' ', '-')
          slug = re.sub(r'[^a-z0-9-]', '', slug.lower())           
          
          if not meta_description or not slug:
              metadata_prompt = f"""
              Generate comprehensive SEO metadata for this recipe: {focused_keyword}
              
              Context:
              - Original keyword: {keyword}
              - Optimized keyword: {focused_keyword}
              - Recipe title: {title}
              
              Create a JSON object with the following fields:
              1. meta_description: An engaging 150-160 character SEO description including the focus keyword
              2. slug: A short, SEO-friendly URL path (max 75 chars) that:
              - Contains the focus keyword
              - Uses hyphens between words
              - Has no special characters or spaces
              - Is all lowercase
              - Is concise but descriptive
              3. schema_type: This should be "Recipe"
              
              Return ONLY a valid JSON object with these exact fields and nothing else.
              """
              
              metadata_response = self.anthropic_client.messages.create(
                  model=self.config.config['claude_model'],
                  max_tokens=800,
                  temperature=1,
              ...
              if not metadata_text.startswith('{'):
                  ...
                  if json_start >= 0 and json_end > json_start:
                      metadata_text = metadata_text[json_start:json_end]
              
              try:
                  claude_metadata = json.loads(metadata_text)
                  
                  if not meta_description:
                      meta_description = claude_metadata.get('meta_description', self._generate_fallback_description(focused_keyword))
                  
                  if not slug:
                      slug = claude_metadata.get('slug', self._generate_slug(title, focused_keyword))
                  
                  self.logger.info(f"Generated metadata via Claude: description={meta_description[:50]}..., slug={slug}")
                  
              except (json.JSONDecodeError, KeyError) as e:
                  self.logger.error(f"Error parsing Claude metadata: {str(e)}")
                  if not meta_description:
                      meta_description = self._generate_fallback_description(focused_keyword)
                  if not slug:
                      slug = self._generate_slug(title, focused_keyword)
          
          rankmath_schema = {
              "@type": "Recipe",
              "name": recipe_card_data.get("Recipe_Title", title),
              "description": recipe_card_data.get("Recipe_Description", meta_description),
              "author": {"@type": "Person", "name": self.config.config.get("author_name", "Jennie Graham")},
              "datePublished": datetime.now().strftime("%Y-%m-%d"),
              "prepTime": recipe_card_data.get("Recipe_Details", {}).get("Prep_Time", "15 minutes"),
              "cookTime": recipe_card_data.get("Recipe_Details", {}).get("Cook_Time", "30 minutes"),
              "totalTime": self._calculate_total_time(
                  recipe_card_data.get("Recipe_Details", {}).get("Prep_Time", "15 minutes"),
                  recipe_card_data.get("Recipe_Details", {}).get("Cook_Time", "30 minutes")
              ),
              "recipeYield": recipe_card_data.get("Recipe_Details", {}).get("Yield", "4 servings"),
              "recipeCategory": recipe_card_data.get("Recipe_Details", {}).get("Category", "Main Course"),
              "recipeCuisine": recipe_card_data.get("Recipe_Details", {}).get("Cuisine", "American"),
              "recipeIngredient": recipe_card_data.get("Recipe_Ingredients", []),
              "recipeInstructions": recipe_card_data.get("Recipe_Instructions", []),
              "nutrition": {
                  "@type": "NutritionInformation",
                  "servingSize": recipe_card_data.get("Recipe_Nutrition", {}).get("Serving_Size", "1 serving"),
                  "calories": recipe_card_data.get("Recipe_Nutrition", {}).get("Calories", "300 calories")
              }
          }
          
          recipe_card_json = json.dumps(recipe_card_data)
          
          serp_source = serp_data.get("source_url", "")
          ...
          return ArticleMetadata(
              ...
              focus_keyword=focused_keyword,
              meta_description=meta_description,
              seo_data=rankmath_schema,
              html_content=content,
              word_count=seo_metadata.get("word_count", 0),
              recipe_card=recipe_card_json,
              serp_source=serp_source,
              featured_image_id=seo_metadata.get("featured_image_id")
          )
      
      except Exception as e:
          self.logger.log_api_call("Metadata Generation", {"keyword": keyword}, error=e)
          
          recipe_card_json = json.dumps(self._create_fallback_recipe_card(keyword, serp_data.get("recipe_data", {})))
          
          return ArticleMetadata(
              title=f"Delicious {focused_keyword.title()} Recipe",
              slug=self._generate_slug(f"Delicious {focused_keyword} Recipe", focused_keyword),
              focus_keyword=focused_keyword,
              meta_description=f"Learn how to make this delicious {focused_keyword} recipe with our step-by-step instructions. Perfect for any occasion!",
              seo_data={"@type": "Recipe", "name": focused_keyword},
              html_content=content,
              word_count=0,
              recipe_card=recipe_card_json
          )
    ...
    async def publish_article(self, website_config: Dict, metadata: ArticleMetadata, featured_image_id: Any, featured_image_url: Any) -> str:
        ...
        try:
            ...
            if metadata.featured_image_id:
                # Set featured image for main post
            ...
            rankmath_fields = [
                ...
                #{"key": "rank_math_seo_score", "value": "100"}
            ]
            ...
            # 5. Add source URL if available - use custom fields for this non-RankMath field
            ...
            if hasattr(metadata, 'recipe_card') and metadata.recipe_card:
                ...
                if recipe_data:
                    try:
                        ...
                        # Prepare recipe meta data
                        recipe_meta = [
                            {"key": "_pinterest_shares", "value": "0"},
                            {"key": "name", "value": recipe_data.get('Recipe_Title', '')},
                            {"key": "title", "value": recipe_data.get('Recipe_Title', '')},
                            {"key": "_total_time", "value": recipe_data.get('Recipe_Details', {}).get('Total_Time', '')},
                            {"key": "_prep_time", "value": recipe_data.get('Recipe_Details', {}).get('Prep_Time', '')},
                            {"key": "_cook_time", "value": recipe_data.get('Recipe_Details', {}).get('Cook_Time', '')},
                            {"key": "_yield", "value": recipe_data.get('Recipe_Details', {}).get('Yield', '')},
                            {"key": "_course", "value": recipe_data.get('Recipe_Details', {}).get('Category', '')},
                            {"key": "_method", "value": recipe_data.get('Recipe_Details', {}).get('Method', '')},
                            {"key": "_cuisine", "value": recipe_data.get('Recipe_Details', {}).get('Cuisine', '')},
                        ]

                        # Add ingredients
                        ingredients_data = recipe_data.get('Recipe_Ingredients', [])
                        if ingredients_data:
                            ingredients_html = "<ul>"
                        ...
                        # Add instructions
                        instructions_data = recipe_data.get('Recipe_Instructions', [])
                        ...
                        if recipe_data.get('Recipe_Notes'):
                            recipe_meta.append({"key": "notes", "value": recipe_data['Recipe_Notes']})
...
class ArticleGenerator:
    ...
    async def generate_article(self, website_config: Dict, keyword: str, progress_tracker: ProgressTracker) -> Dict:
        try:
            ...
            serp_data = await self._execute_step(
                "SERP ANALYSIS",
                self.keyword_processor.analyze_serp,
                keyword
            )
            
            # Step 3: Keyword Optimization
            focused_keyword = await self._execute_step(
            ...
            # Step 6: External links research
            ...
            # Step 7: Internal links research
            ...
            # FIXED: Create WordPress client ONLY when needed (moved to image generation step)
            # Don't create it here as it might timeout during long content generation
            
            # Step 9: Image generation with WordPress client
            try:
                # Create client right before using it for images
                client = Client(
                    website_config['xmlrpc_url'],
                    website_config['username'],
                    website_config['api_secret']
                ...
                content_with_images, featured_image_id, featured_image_url = await self._execute_step(
                    "IMAGE GENERATION",
                ...
                # Update content if images were added
                if content_with_images:
                    content = content_with_images
                    self.logger.info("✅ Images successfully added to content")
                else:
                    self.logger.warning("⚠️ No images were added to content")

                # FIXED: Add featured image ID to metadata properly
                if featured_image_id:
                    content_seo_metadata["featured_image_id"] = featured_image_id
                    self.logger.info(f"✅ Featured image ID set: {featured_image_id}")
                else:
                    self.logger.warning("⚠️ No featured image ID was generated")
                    
            except Exception as image_error:
                self.logger.error(f"❌ Image generation failed: {str(image_error)}")
                # Continue without images rather than failing completely
                featured_image_id = None
                featured_image_url = None
                content_seo_metadata["featured_image_id"] = None

            # Step 10: Recipe card generation
            try:
                recipe_card_result = await self._execute_step(
                    "RECIPE CARD GENERATION",
                    self.content_generator.generate_recipe_card,
                    content,
                    focused_keyword,
                    serp_data.get("recipe_data", {})
                )
                
                # FIXED: Handle None result properly
                if recipe_card_result is None:
                    recipe_card_result = {}
                    self.logger.warning("⚠️ Recipe card generation returned None")
                else:
                    self.logger.info("✅ Recipe card generated successfully")
                    
            except Exception as recipe_error:
                self.logger.error(f"❌ Recipe card generation failed: {str(recipe_error)}")
                recipe_card_result = {}

            # Extract SEO metadata from recipe card
            recipe_seo_metadata = recipe_card_result.get("SEO_Metadata", {})

            # FIXED: Create definitive SEO metadata with better fallbacks
            definitive_seo_metadata = {
                "focus_keyword": recipe_seo_metadata.get("focus_keyword", focused_keyword),
                "seo_title": recipe_seo_metadata.get("seo_title", ""),
                "meta_description": recipe_seo_metadata.get("seo_description", ""),
                "slug": recipe_seo_metadata.get("slug", ""),
                "word_count": content_seo_metadata.get("word_count", 0),
                "featured_image_id": featured_image_id,
                "recipe_card": recipe_card_result
            }
            
            self.logger.info(f"📊 SEO Metadata: slug='{definitive_seo_metadata['slug']}', word_count={definitive_seo_metadata['word_count']}")
            ...
            result = {
                ...
                "serp_source": getattr(metadata, 'serp_source', '')  # FIXED: Handle missing attribute safely
            }
            
            # FIXED: Update progress as completed with proper success tracking
            progress_tracker.update_progress(website_config['name'], keyword, "completed", result)
            
            # FIXED: Reset current processing info
            self.current_keyword = None
            self.current_website_config = None
            
            self.logger.info(f"✅ COMPLETED: '{keyword}' for '{website_config['name']}' - {article_url}")
            
            return result

        except Exception as e:
            # FIXED: Better error handling and logging
            error_msg = f"Error generating article for keyword '{keyword}': {str(e)}"
            self.logger.error(f"❌ {error_msg}")
            
            # Update progress as failed with error details
            progress_tracker.update_progress(
                website_config['name'], 
                keyword, 
                "failed",
                {"error": str(e), "current_step": f"Failed at: {getattr(self, '_current_step', 'unknown')}"} 
            )
            
            # FIXED: Reset current processing info even on failure
            self.current_keyword = None
            self.current_website_config = None
            
            # Re-raise with the original exception
            raise

    async def generate_basic_images(self, website_config, keyword, focused_keyword, content, image_source_url, client):
        ...
        try:
            ...
            # Step 4: Upload featured image with retry logic
            ...
            # Step 5: Image placement - Place images at specific locations
            ...
            if remaining_images:
                ...
                for i, image_url in enumerate(images_to_place):
                    try:
                        ...
                        title = f"{focused_keyword} Recipe Image {i+1}"
    ...
    async def _upload_to_wordpress_with_retry(self, client, image_data, title, alt_text, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                image_name = f"{safe_title[:20]}-{random_suffix}.jpg"
...
async def start_all_websites():
    """Start processing for ALL active websites individually"""
    try:
        started_websites = []
        failed_websites = []
        ...
        for website in websites:
            if website.get('is_active', True):
                try:
                    # Start each website individually (same logic as individual start)
                    website_name = website['name']
                    
                    if progress_tracker.is_website_processing(website_name):
                        continue  # Skip if already processing
                    
                    # Create background task for this website
                    ...
                    # Start tracking for this website
                    progress_tracker.start_website(website_name)
                    ...
                    started_websites.append(website_name)
                    logger.info(f"Started processing for website: {website_name}")
                    
                except Exception as e:
                    failed_websites.append({"website": website['name'], "error": str(e)})
                    logger.error(f"Failed to start {website['name']}: {str(e)}")
        
        return {
            "status": "success",
            "message": f"Started processing for {len(started_websites)} websites",
            "data": {
                "started": started_websites,
                "failed": failed_websites,
                "total_started": len(started_websites)
            }
        }
        
    except Exception as e:
        logger.error(f"Error in start_all_websites: {str(e)}")
