...
class Config:
    ...
    def _load_environment_variables(self):
        ...
        for config_key, env_key in env_mapping.items():
            ...
            if env_value:
                ...
                print(f"✅ Loaded {config_key} from environment variable {env_key}")
        
        # Handle nested discord token
        discord_token = os.getenv('DISCORD_TOKEN')
        if discord_token:
            if 'discord' not in self.config:
                self.config['discord'] = {}
            self.config['discord']['token'] = discord_token
            print(f"✅ Loaded discord token from environment variable DISCORD_TOKEN")
        
        # Handle nested recraft API key
        recraft_key = os.getenv('RECRAFT_API_KEY')
        if recraft_key:
            if 'recraft' not in self.config:
                self.config['recraft'] = {}
            self.config['recraft']['api_key'] = recraft_key
            print(f"✅ Loaded recraft API key from environment variable RECRAFT_API_KEY")
    ...
    def _validate_config(self):
        ...
        # Validate that sensitive keys are loaded
        sensitive_keys = ['claude_api_key', 'serpapi_api_key']
        for key in sensitive_keys:
            if not self.config.get(key) or self.config[key] in ['', 'your-key-here', 'PLACEHOLDER']:
                env_key = key.upper()
                env_value = os.getenv(env_key)
                if not env_value:
                    raise ConfigError(f"Missing or invalid {key}. Please set {env_key} in your .env file or environment variables.")
                
        print("🔐 All required API keys validated successfully!")
        print(f"📊 Claude API Key: {'✅ Loaded' if self.config.get('claude_api_key') else '❌ Missing'}")
        print(f"🔍 SERP API Key: {'✅ Loaded' if self.config.get('serpapi_api_key') else '❌ Missing'}")
        print(f"🎮 Discord Token: {'✅ Loaded' if self.config.get('discord', {}).get('token') else '❌ Missing'}")
        print(f"🎨 Recraft API Key: {'✅ Loaded' if self.config.get('recraft', {}).get('api_key') else '❌ Missing'}")
...
# Progress Tracker class - FIXED VERSION
class ProgressTracker:
    ...
    def start_website(self, website_name: str):
        # ALWAYS reset stats when starting
        ...
        # Clear any existing progress for this website
        keys_to_remove = [key for key in self._progress.keys() if key.startswith(f"{website_name}::")]
    ...
    def update_progress(self, website: str, keyword: str, status: str, metadata: Optional[Dict] = None):
        """Thread-safe progress update with proper synchronization"""
        try:
            ...
            # Add keyword to tracking list if not already there
            if keyword and keyword not in self._website_stats[website]["keywords"]:
                self._website_stats[website]["keywords"].append(keyword)
                self.logger.info(f"Added keyword '{keyword}' to {website} tracking list")
            
            # Update keyword progress in _progress dictionary
            if keyword:
                self._progress[key] = {
                    "website": website,
                    "keyword": keyword,
                ...
                self.logger.info(f"Updated progress for {key}: status={status}")
            
            # Clear current keyword when completed or failed
            if status in ["completed", "failed"] and keyword:
                if self._website_stats[website].get("current_keyword") == keyword:
                    self._website_stats[website]["current_keyword"] = None
                    self._website_stats[website]["current_step"] = None
    ...
    def _update_website_progress(self, website_name: str):
        """Recalculate progress for a website with proper error handling"""
        try:
            if website_name not in self._website_stats:
                self.logger.warning(f"Website {website_name} not in stats")
                return
            ...
            for keyword in stats.get("keywords", []):
                ...
                if key in self._progress:
                    keyword_status = self._progress[key].get("status", "")
                    if keyword_status == "completed":
                        completed += 1
                    elif keyword_status == "failed":
                        failed += 1
                    elif keyword_status == "processing":
                        in_progress += 1
                        
                    self.logger.debug(f"Keyword '{keyword}' status: {keyword_status}")
                else:
                    self.logger.warning(f"Key {key} not found in progress tracking")
            ...
            if total > 0:
                progress_percent = min(100, int(((completed + failed) / total) * 100))
                stats["progress"] = progress_percent
            else:
                stats["progress"] = 0
                
            # Log the update
            self.logger.info(f"Updated {website_name} progress: {completed}/{total} completed, "
                            f"{failed} failed, {in_progress} in progress, {stats['progress']}%")
                            
        except Exception as e:
            self.logger.error(f"Error in _update_website_progress: {str(e)}")
            self.logger.exception(e)
    ...
    def get_website_status(self, website_name: str) -> Optional[Dict]:
        ...
        if stats.get("completed", 0) > stats.get("total_keywords", 0):
            stats["completed"] = stats.get("total_keywords", 0)
    ...
    async def generate_article_content(self, keyword: str, focused_keyword: str, outline: List[Dict], 
      ...
      try:
          ...
          prompt = f"""
           ...
           One important thing to do is, the article should focus on the intent approach used by google seo to define the quality of the article and rank them based on it, you should 
           write an article that keeps the user interested from the jump (the very first paragraph) and wanting to read more, therefore focus on health benefits for example, insightful infos,
           the users want mainly to want about the recipe, the ingredients, the recipe cost range, the health benefits, when to cook it (family gatherings ? occasions?), keep the user hooked, and don't diverge
           from the aim of the article, we want to provide the reader with everything interesting about the recipe.
...
auth_manager = None
background_tasks = {}
background_tasks_by_website = {}  # Track tasks by website name for better management
...
async def start_all_websites():
    ...
    try:
        global progress_tracker, background_tasks_by_website
        
        # Cancel all existing background tasks first
        logger.info("Cancelling all existing background tasks...")
        for website_name, task in background_tasks_by_website.items():
            if not task.done():
                task.cancel()
                logger.info(f"Cancelled task for website: {website_name}")
        
        # Clear the task dictionaries
        background_tasks_by_website.clear()
        ...
        return {
            ...
            "data": {
                ...
                "cancelled_tasks": len(background_tasks_by_website)
            }
        }
...
async def start_website_processing(website_name: str, current_user: dict = Depends(get_current_user)):
   try:
       ...
       # Cancel existing task for this website if any
       if website_name in background_tasks_by_website:
           old_task = background_tasks_by_website[website_name]
           if not old_task.done():
               old_task.cancel()
               logger.info(f"Cancelled previous task for website: {website_name}")
       
       if progress_tracker.is_website_processing(website_name):
           # Force reset if already processing
           logger.warning(f"Website {website_name} was already processing, resetting...")
       
       task_id = f"{website_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
       
       # IMPORTANT: Reset website stats before starting
       progress_tracker.start_website(website_name)
       ...
       background_tasks_by_website[website_name] = task
...
async def stop_website_processing(website_name: str, current_user: dict = Depends(get_current_user)):
   try:
       ...
       # Also remove from website-specific tracking
       if website_name in background_tasks_by_website:
           del background_tasks_by_website[website_name]
...
async def get_website_progress(website_name: str, current_user: Optional[dict] = Depends(get_current_user_optional)):
    """Get real-time progress for a specific website"""
    try:
        ...
        logger.info(f"📈 {website_name} stats: completed={status.get('completed', 0)}, "
                   f"total={status.get('total_keywords', 0)}, "
...
async def process_website_background(website_name: str, website_config: dict, task_id: str):
    ...
    try:
        ...
        progress_tracker.update_progress(website_name, "", "processing", {"current_step": "Loading Google Sheets data"})
        ...
        for index, row in processing_rows.iterrows():
            ...
            if keyword in processed_keywords:
                logger.warning(f"Skipping duplicate keyword '{keyword}' for {website_name}")
                continue
